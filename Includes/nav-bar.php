<div class="m-div">
    <img src="http://saraswati-ktpatelschool.org/wp-content/uploads/2017/01/sarswati-vidhalay-logoooo-png.png" alt="School-Logo" class="S-Logo" width="150" height="120">
<nav>
    <div class="toggle">
    <i class="fa fa-bars menu" aria-hidden="true"></i>
    </div>
<ul>
    <li class="m-l"><a href="#">Home</a></li>
    <li><a href="#">Activity</a></li>
    <li><a href="#">Contact</a></li>
    <li><a href="#">About</a></li>
    <li id="S-links"><a href="#" class="S-link"><i class="fa fa-facebook" aria-hidden="true"></i></a> 
    <a href="#" class="S-link"><i class="fa fa-google-plus" aria-hidden="true"></i></a> 
    <a href="#" class="S-link"><i class="fa fa-twitter" aria-hidden="true"></i></a></li>
</ul>
</nav>
</div>
<script src="https://code.jquery.com/jquery-3.3.1.js"></script>
<script type="text/javascript">
    $(document).ready(function(){
        $('.menu').click(function(){
            $('ul').toggleClass('active')
        })
    })
</script>